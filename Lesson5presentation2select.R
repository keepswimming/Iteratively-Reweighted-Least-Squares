...
bodyfat.orig = read.csv("bodyfat-2.csv")
#View(bodyfat.orig)
...
bodyfat <- bodyfat.orig %>% #piped it to get rid on the predictor variables that are not needed
...
# fit models
x = model.matrix(BodyFatSiri~.,data=bodyfat)[,-1]#exclude the 1st column, which is a column of 1's
...
#lambdalist = exp((-1200:1200)/100)  # denser array covering a large amt of possible values then trim it down
...
cvRRglmnet$lambda.min; log(cvRRglmnet$lambda.min) #min lambda
cvRRglmnet$lambda.1se; log(cvRRglmnet$lambda.1se) #within 1SE
min(cvRRglmnet$cvm) #min cv values, from $cvm
